...
from seleniumwire.webdriver import (ChromeOptions,  # type: ignore
                                    FirefoxOptions)
...
class Telenium:
    """
    Telenium is a self-cleaning selenium browser automation tool.
    """
    ...
    def __enter__(self) -> WebDriver:
        ...
        logger.info(
            f"Cleanup pid for {self._browser_launcher.name} is {self._cleanup_pid}"
        )
    ...
    def __exit__(
        ...
        if exc is None:
            logger.info(
                f"... Quitting browser and killing cleanup process {self._cleanup_pid}"
            )
            self._browser.quit()
    ...
    def _start_cleanup(self) -> int:
        process = Popen(
            [
                "python",
                "telenium/src/utils/_cleanup.py",
                str(self._timeout),
